var ws = new WebSocket('wss://' + location.host + ':8443/one2one');
var videoInput = [];
var videoOutput = [];
var webRtcPeer = [];
...
function register(name_user) {
    ...
    document.getElementById('my_video_block').classList.remove("videoroom_hidden");
    var name = name_user;
    if (name == '') {
...
function call(from, to) {
    ...
    if (finduserStatus[to]) {
        ...
        if (to == $video_translater) {
            document.getElementById('translator_video_block').classList.remove("videoroom_hidden");
            var options = {
        ...
        webRtcPeer[from] = kurentoUtils.WebRtcPeer.WebRtcPeerSendrecv(options, function (error) {
            if (error) {
                // console.error(error);
            }
...
function stop(name) {
    ...
    if (name) {
        ...
        if (name == $video_user2) {
            document.getElementById('translator_video_block').classList.add("videoroom_hidden");
        }
...
function initcamfunc() {
    if (initcam == false) {
        ...
        webRtcPeer['local'] = kurentoUtils.WebRtcPeer.WebRtcPeerSendrecv(options, function (error) {
